...
<p align="center">
  <a href="https://github.com/wakelinkdev/ewsp-core/releases"><img src="https://img.shields.io/badge/version-1.0.0-blue.svg" alt="Version"></a>
  <a href="https://github.com/wakelinkdev/ewsp-core/actions"><img src="https://img.shields.io/badge/build-passing-brightgreen.svg" alt="Build"></a>
  <a href="LICENSE"><img src="https://img.shields.io/badge/license-NGC%20v1.0-orange.svg" alt="License"></a>
...
| XChaCha20 | `ewsp_xchacha20()` | Extended nonce (24 bytes) |

### Security Fixes (v3.0)

- **CRYPTO-01**: Удален unsafe `rand()` fallback для RNG
- **CRYPTO-02**: Только hardware RNG (esp_fill_random)
- **CRYPTO-03**: Key separation через HKDF (разные ключи для encryption/auth)
- **CRYPTO-04/05**: Constant-time comparison против timing attacks
...
│   ├── ewsp_chain.h           # Blockchain chain state
...
|----------|---------------|------------|
| SHA-256 | Хэширование, Key derivation | Встроенная |
| HMAC-SHA256 | Аутентификация пакетов | Встроенная |
| HKDF-SHA256 | Расширение ключей | Встроенная |
| ChaCha20 | Шифрование (12B nonce) | Встроенная |
| XChaCha20 | Шифрование (24B nonce) | Встроенная |

### Key Derivation

```
device_token (≥32 chars)
        │
        ▼
    SHA-256
        │
...
```
┌────────────┬──────────────────────┬────────────────┐
│ 2B length  │ XChaCha20 ciphertext │ 24B nonce      │
│ (big-end)  │ (variable)           │                │
└────────────┴──────────────────────┴────────────────┘
```
...
| Encrypt 1KB | 890 µs | 35 µs | 25x |
| Create packet | 1.2 ms | 50 µs | 24x |
| Process packet | 1.5 ms | 60 µs | 25x |
